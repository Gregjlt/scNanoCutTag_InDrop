...
demultiplex_nanobc_func()
{
  ...
  select_lines.sh ${output_dir}/fastqs/${name}.lines_to_keep.txt ${index} 1> ${output_dir}/fastqs/${name}.R2.fastq 2> ${log}
  select_lines.sh ${output_dir}/fastqs/${name}.lines_to_keep.txt ${forward} 1> ${output_dir}/fastqs/${name}.R1.fastq 2> ${log}
  select_lines.sh ${output_dir}/fastqs/${name}.lines_to_keep.txt ${reverse} 1> ${output_dir}/fastqs/${name}.R3.fastq 2> ${log}
...
remove_nanobc_read1N_func()
{
  ...
  zcat ${index} | awk 'NR%4==2 || NR%4==0{print substr($0,34)} NR%4!=2 && NR%4!=0{print}' | gzip > ${output_dir}/fastqs/${name}.R2.fastq.temp.gz
...
reverse_fastq_func()
{
    ...
    rm ${output_dir}/fastqs/${name}.R2.fastq.temp.gz
   echo
